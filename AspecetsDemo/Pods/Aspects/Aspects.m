...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self);
	Class statedClass = self.class; // class 被重写了，返回hook（isa swizzled）之前的类，对外声明的class，原来的class，与实际的class可能不一致。实际的class是isa指向的Class，通过object_getClass
	Class baseClass = object_getClass(self); // 返回类实际指向的isa
	...
	if (subclass == nil) {
		subclass = objc_allocateClassPair(baseClass, subclassName, 0); // 创建一个新的类，被hook类的子类
		if (subclass == nil) {
		...
		aspect_swizzleForwardInvocation(subclass); // 对被hook类的子类的forwardInvocation:进行method swizzle，替换为__ASPECTS_ARE_BEING_CALLED__
		aspect_hookedGetClass(subclass, statedClass); // 重写被hook类的子类的的class方法，使其返回被hook的类
		aspect_hookedGetClass(object_getClass(subclass), statedClass);
		objc_registerClassPair(subclass);
	}

	object_setClass(self, subclass); // 对self进行isa swizzle，使其isa指向刚被创建的子类。[self class]返回hook之前的类，objc_getClass(self),返回当前isa指向的类。
	return subclass;
}
...
- (BOOL)removeAspect:(id)aspect {
    // aspect 为要删除的aspect，依次从三个数组中查找并删除之。
